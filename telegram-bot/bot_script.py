...
def error(update, context):
    ...
    context.bot.send_message(config.admin_id, str(context.error))
...
def addtime(update, context):
    if update.message.chat_id == config.group_id:
        ...
        if len(value[2]) == 0:
            ...
            update.message.reply_text("Try including a time")
        else:
            first = value[0]
            if len(first) == 0:
                first = "0"
...
def populate_database(update, context):
    ...
    if update.message.from_user.id == config.admin_id:
        crosswars.create_db_from_scratch(globalChatData)
